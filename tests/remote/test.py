import random
import time
from tests.client.utils import create_client_env, exec_esc_cmd, get_balance_user, create_account
...
def test_send_money(count=100):
    ...
    assert float(finish_balance_1) == float(start_balance_1) - sum(sent_user_1) + sum(sent_user_2)
    assert float(finish_balance_2) == float(start_balance_2) - sum(sent_user_2) + sum(sent_user_1)