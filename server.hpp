...
public:
	...
	server(options& opts) :
			do_sync(1), do_fast(1), endpoint_(boost::asio::ip::tcp::v4(),
					opts.port),	//TH
			io_service_(), work_(io_service_), acceptor_(io_service_,
					endpoint_), opts_(opts), do_validate(0), votes_max(0.0), do_vote(
					0), do_block(0)
	//ofip(NULL)
	{
		...
		if (!last_srvs_.find_key(pkey, skey)) {
			...
			fprintf(stderr,
					"ERROR: failed to find secret key for key:\n%.16s\n",
					pktext);
			exit(-1);
		}
		...
		if (!undo_bank()) { //check database consistance, if
			if (!opts_.fast) {
				std::cerr
						<< "DATABASE check failed, must use fast option to load new datase from network\n";
				exit(-1);
			}
			do_sync = 1;
		} else {
			...
			if (last_srvs_.now == now - BLOCKSEC) {
				...
				if (last_srvs_.now < now - MAXLOSS && !opts_.fast) {
					std::cerr
							<< "WARNING, possibly missing too much history for full resync\n";
				}
				do_sync = 1;
			}
		}
		...
		} //wait some time before connecting to more peers
...
//FIXME, must sign this message again if message too old !!!
			message_ptr msg(
					new message(MSGTYPE_TXS, lastpath, opts_.svid, lastmsid,
							pkey, msha)); //load from file
			if (msg->status != MSGSTAT_DAT) {
				fprintf(stderr,
						"ERROR, failed to read message %08X/%02x_%04x_%08x.txt\n",
			...
			if (txs_insert(msg)) {
				fprintf(stderr,
						"RECYCLED message %04X:%08X from %08X/ inserted\n",
						opts_.svid, lastmsid, lastpath);
				if (srvs_.now != lastpath) {
					fprintf(stderr,
							"MOVE message %04X:%08X from %08X/ to %08X/\n",
	...
	void mklogfile(uint16_t svid, uint32_t user) {
		...
		mkdir(filename, 0755);
		sprintf(filename, "log/%04X/%03X/%03X", svid, user >> 20,
				(user & 0xFFF00) >> 8);
		mkdir(filename, 0755);
		//create log file ... would be created later anyway
		sprintf(filename, "log/%04X/%03X/%03X/%02X.log", svid, user >> 20,
				(user & 0xFFF00) >> 8, (user & 0xFF));
	...
	int purge_log(int fd, uint32_t user) // this is ext4 specific !!!
		...
		for (; size >= LOG_PURGE_START; size -= 4096) {
			if (lseek(fd, 4096 - sizeof(log_t), SEEK_SET)
					!= 4096 - sizeof(log_t)) {
				unlock_user(user);
	...
	void put_log(uint16_t svid, std::map<uint64_t, log_t>& log) {
		...
		for (auto it = log.begin(); it != log.end(); it++) {
			...
			if (luser != user) {
				...
				luser = user;
				char filename[64];
				sprintf(filename, "log/%04X/%03X/%03X/%02X.log", svid,
						user >> 20, (user & 0xFFF00) >> 8, (user & 0xFF));
				fd = open(filename, O_WRONLY | O_CREAT | O_APPEND, 0644); //maybe no lock needed with O_APPEND
				if (fd < 0) {
					std::cerr << "ERROR, failed to open log register "
							<< filename << "\n";
					return;
...
//TODO, monotonic time increase would help
	...
	bool fix_log(uint16_t svid, uint32_t user) {
		...
		fstat(wd, &sb);
		if (sb.st_size % sizeof(log_t)) { // file is ok
			std::cerr << "ERROR, failed to append log register " << filename
					<< ", why ???\n";
			close(rd);
			close(wd);
			return (false);
		...
		for (l = sizeof(log_t); l < sb.st_size; l += sizeof(log_t)) {
			if (read(rd, &log, sizeof(log_t)) != sizeof(log_t)) {
				std::cerr << "ERROR, failed to read log register " << filename
						<< " while fixing\n";
				break;
			}
			if (write(wd, &log, sizeof(log_t)) != sizeof(log_t)) {
				std::cerr << "ERROR, failed to write log register " << filename
						<< " while fixing\n";
				break;
			}
		}
	...
	bool get_log(uint16_t svid, uint32_t user, uint32_t from,
			std::string& slog) {
		char filename[64];
		struct stat sb;
		if (from == 0xffffffff) {
		...
		int l;
		for (uint32_t tot = 0;
				(l = read(fd, &log, sizeof(log_t))) > 0 && tot < len; tot +=
						l) {
			if (log.time < from) {
				continue;
			}
	...
	{ //could use multiple threads but disk access could limit the processing anyway
		...
		for (uint16_t bank = 1; bank < last_srvs_.nodes.size(); bank++) {
			...
			for (uint32_t user = 0; user < users; user++) {
				...
				if (ud >= 0) {
					u.msid = 0;
					if (sizeof(user_t) == read(ud, &u, sizeof(user_t))
							&& u.msid) {
						//std::cerr<<"OVERWRITE: "<<bank<<":"<<user<<" ("<<u.weight<<")\n";
						fprintf(stderr,
								"OVERWRITE: %04X:%08X (weight:%016lX)\n", bank,
								user, u.weight);
			...
			if (last_srvs_.nodes[bank].weight != weight) {
				...
				fprintf(stderr,
						"ERROR loading bank %04X (bad sum:%016lX<>%016lX)\n",
	...
	void load_chain() { //char pathname[16];
		uint32_t now = time(NULL);
		now -= now % BLOCKSEC;
		do_validate = 1;
...
//FIXME, must start with a matching nowhash and load serv_
		for (auto block = headers.begin(); srvs_.now < now;) {
			...
			if (block == headers.end()) { // wait for peers to load more blocks
				...
				if (block != headers.begin()) {
					...
					fprintf(stderr,
							"WAITING for headers %08X, maybe need more peers\n",
			...
			for (auto it = missing_msgs_.begin(); it != missing_msgs_.end();) {
				...
				if (jt->second->load()) {
					if (!jt->second->sigh_check(jt->second->data + 4)) {
						...
						fprintf(stderr,
								"LOADING TXS %04X:%08X from path:%08X\n",
						...
						fprintf(stderr,
								"LOADING TXS %04X:%08X from path:%08X failed\n",
								jt->second->svid, jt->second->msid,
								jt->second->path);
						jt->second->len = message::header_length;
					}
					continue;
				} // assume no lock needed
				fillknown(jt->second);
			...
			} //also calculates weight of the node
			//finish block
			...
			if (memcmp(srvs_.nowhash, block->nowhash, SHA256_DIGEST_LENGTH)) {
				...
				fprintf(stderr,
						"ERROR, failed to arrive at correct hash at block %08X, fatal\n",
						srvs_.now);
				exit(-1);
			}
	...
	void add_headers(std::vector<servers>& peer_headers) {
		...
		peer_.lock();
		if (!headers.size()) {
			headers.insert(headers.end(), peer_headers.begin(),
					peer_headers.end());
			peer_.unlock();
			return;
		}
		auto it = peer_headers.begin();
		for (; it != peer_headers.end() && it->now <= headers.back().now;
				it++) {
		}
	...
	void message_msha(std::map<uint16_t, message_ptr>& map) {
		svid_msha.clear();
		for (std::map<uint16_t, message_ptr>::iterator it = map.begin();
				it != map.end(); ++it) {
			msidhash_t msha;
	...
	void message_shash(uint8_t* mhash, std::map<uint16_t, message_ptr>& map) {
		...
		for (std::map<uint16_t, message_ptr>::iterator it = map.begin();
			...
			fprintf(stderr, "SHASH: %d:%d %.*s\n", (int) (it->first),
					(int) (it->second->msid), 2 * SHA256_DIGEST_LENGTH, sigh);
	...
	void count_votes(uint32_t now, hash_s& cand) // cand_.locked()
		...
		if (do_block < 2
				&& ((num1->score
						> (num2 != NULL ? num2->score : 0)
								+ (votes_max - votes_counted))
			...
			if (now > srvs_.now + BLOCKSEC + MAX_ELEWAIT) {
				...
				fprintf(stderr,
						"CANDIDATE ELECTED:%016lX second:%016lX max:%016lX counted:%016lX\n",
						num1->score, x, votes_max, votes_counted);
			}
			do_block = 2;
			winner = num1;
		...
		if (do_vote
				&& now > srvs_.now + BLOCKSEC + (do_vote - 1) * VOTE_DELAY) {
			std::cerr << "CANDIDATE proposing\n";
			write_candidate(cand);
		}
	}
	...
	void prepare_poll() // select CANDIDATE_MAX candidates and VOTES_MAX electors
	{
		...
		for (auto it = last_svid_msgs.begin(); it != last_svid_msgs.end();
				++it) {
		...
		if (!svid_rank.size()) {
			std::cerr
					<< "ERROR, no valid server for this block :-(, TODO create own block\n";
		} else {
			//std::cerr << "SORT \n";
			std::sort(svid_rank.begin(), svid_rank.end(),
					[this](const uint16_t& i,const uint16_t& j) {return(this->srvs_.nodes[i].weight>this->srvs_.nodes[j].weight);});
		} //fuck, lambda :-/
		//TODO, save this list
		for (uint32_t j = 0; j < VOTES_MAX && j < svid_rank.size(); j++) {
			...
			fprintf(stderr, "ELECTOR[%d]=%016lX\n", svid_rank[j],
					srvs_.nodes[svid_rank[j]].weight);
	...
	message_ptr message_svidmsid(uint16_t svid, uint32_t msid) {
		...
		while (mi != txs_msgs_.end() && mi->second->svid == svid
				&& mi->second->msid == msid) {
	...
	message_ptr message_find(message_ptr msg, uint16_t svid) {
		fprintf(stderr, "HASH find:%016lX (%04X%08X) %d:%d\n", msg->hash.num,
				msg->svid, msg->msid, msg->svid, msg->msid);
		if (msg->data[0] == MSGTYPE_GET) {
			txs_.lock();
			std::map<uint64_t, message_ptr>::iterator it =
					txs_msgs_.lower_bound(msg->hash.num & 0xFFFFFFFFFFFFFF00L);
			while (it != txs_msgs_.end()
					&& ((it->first & 0xFFFFFFFFFFFFFF00L)
							== (msg->hash.num & 0xFFFFFFFFFFFFFF00L))) {
				if (it->second->len > 4 + 64
						&& msg->hash.dat[0] == it->second->hashval(svid)) { //data[4+(svid%64)]
					txs_.unlock();
					return it->second;
				}
				it++;
		...
		if (msg->data[0] == MSGTYPE_CNG) {
			cnd_.lock();
			std::map<uint64_t, message_ptr>::iterator it =
					cnd_msgs_.lower_bound(msg->hash.num & 0xFFFFFFFFFFFFFF00L);
			while (it != cnd_msgs_.end()
					&& ((it->first & 0xFFFFFFFFFFFFFF00L)
							== (msg->hash.num & 0xFFFFFFFFFFFFFF00L))) {
				if (it->second->len > 4 + 64
						&& msg->hash.dat[0] == it->second->hashval(svid)) { //data[4+(svid%64)]
			...
			for (auto me = cnd_msgs_.begin(); me != cnd_msgs_.end(); me++) {
				fprintf(stderr, "HASH have: %016lX (%02X)\n", me->first,
						me->second->hashval(svid));
			} //data[4+(svid%64)]
			return NULL;
		}
		if (msg->data[0] == MSGTYPE_BLG) {
			blk_.lock();
			std::map<uint64_t, message_ptr>::iterator it =
					blk_msgs_.lower_bound(msg->hash.num & 0xFFFFFFFFFFFFFF00L);
			while (it != blk_msgs_.end()
					&& ((it->first & 0xFFFFFFFFFFFFFF00L)
							== (msg->hash.num & 0xFFFFFFFFFFFFFF00L))) {
				if (it->second->len > 4 + 64
						&& msg->hash.dat[0] == it->second->hashval(svid)) { //data[4+(svid%64)]
		...
		if (msg->data[0] == MSGTYPE_DBG) {
			dbl_.lock();
			std::map<uint64_t, message_ptr>::iterator it =
					dbl_msgs_.lower_bound(msg->hash.num & 0xFFFFFFFFFFFFFF00L);
			while (it != dbl_msgs_.end()
					&& ((it->first & 0xFFFFFFFFFFFFFF00L)
							== (msg->hash.num & 0xFFFFFFFFFFFFFF00L))) {
				if (it->second->len > 4 + 64) {
					dbl_.unlock();
...
// preh !!!
	...
	int dbl_insert(message_ptr msg) // WARNING !!! it deletes old message data if len==message::header_length
		...
		if (it != dbl_msgs_.end()) {
			...
			if (msg->len > message::header_length
					&& osg->len == message::header_length) { // insert full message
				if (do_sync
						&& memcmp(osg->sigh, msg->sigh, SHA256_DIGEST_LENGTH)) {
					dbl_.unlock();
					std::cerr
							<< "ERROR, getting message with wrong signature hash\n";
					return (0);
				}
				osg->update(msg);
				osg->path = srvs_.now;
	...
	int cnd_insert(message_ptr msg) // WARNING !!! it deletes old message data if len==message::header_length
		...
		fprintf(stderr, "HASH insert:%016lX (CND) [len:%d]\n", msg->hash.num,
				msg->len);
		std::map<uint64_t, message_ptr>::iterator it = cnd_msgs_.find(
				msg->hash.num);
		if (it != cnd_msgs_.end()) {
			message_ptr osg = it->second;
			if (msg->len > message::header_length
				...
				if (pre != NULL && pre->len > message::header_length
						&& (pre->hash.num & 0xFFFFFFFFFFFF0000L)
								== (osg->hash.num & 0xFFFFFFFFFFFF0000L)) {
					create_double_spend_proof(pre, osg); // should copy messages from this server to ds_msgs_
					return (1);
				}
				if (nxt != NULL && nxt->len > message::header_length
						&& (nxt->hash.num & 0xFFFFFFFFFFFF0000L)
								== (osg->hash.num & 0xFFFFFFFFFFFF0000L)) {
	...
	int blk_insert(message_ptr msg) // WARNING !!! it deletes old message data if len==message::header_length
		...
		std::map<uint64_t, message_ptr>::iterator it = blk_msgs_.find(
				msg->hash.num);
		if (it != blk_msgs_.end()) {
			message_ptr osg = it->second;
			if (msg->len > message::header_length
	...
	int txs_insert(message_ptr msg) // WARNING !!! it deletes old message data if len==message::header_length
		...
		std::map<uint64_t, message_ptr>::iterator it = txs_msgs_.find(
				msg->hash.num);
		if (it != txs_msgs_.end()) {
			message_ptr osg = it->second;
			...
			if (msg->len > message::header_length
				...
				if (do_sync
					...
					txs_.unlock();
					fprintf(stderr,
							"HASH insert:%016lX (TXS) [len:%d] WRONG SIGNATURE HASH!\n",
							msg->hash.num, msg->len);
					return (0);
				}
				osg->update(msg);
				...
				if (!osg->save()) { //FIXME, change path
					fprintf(stderr,
							"HASH insert:%016lX (TXS) [len:%d] SAVE FAILED, ABORT!\n",
							msg->hash.num, msg->len);
					exit(-1);
				}
				...
				if (!do_sync) {
					...
					if (pre != NULL && pre->len > message::header_length
							&& (pre->hash.num & 0xFFFFFFFFFFFF0000L)
									== (osg->hash.num & 0xFFFFFFFFFFFF0000L)) {
						fprintf(stderr,
								"HASH insert:%016lX (TXS) [len:%d] DOUBLE SPEND!\n",
								msg->hash.num, msg->len);
						create_double_spend_proof(pre, osg); // should copy messages from this server to ds_msgs_
						return (1);
					}
					...
					if (osg->now >= srvs_.now + BLOCKSEC) {
						fprintf(stderr,
								"HASH insert:%016lX (TXS) [len:%d] delay to %08X/ ???\n",
			...
			if (msg->len == message::header_length) {
				...
				txs_.unlock();
				fprintf(stderr,
						"HASH insert:%016lX (TXS) [len:%d] set as missing\n",
						msg->hash.num, msg->len);
				missing_msgs_insert(msg);
				return (1);
			}
			if (msg->svid == opts_.svid) { // own message
				...
				txs_.unlock();
				fprintf(stderr,
						"HASH insert:%016lX (TXS) [len:%d] store as own\n",
						msg->hash.num, msg->len);
				msg->save();
				check_.lock();
...
//FIXME, check what block are we dealing with !!!
		...
		while (do_validate) {
			...
			if (check_msgs_.empty()) {
				...
				for (auto mj = missing_msgs_.begin(); mj != missing_msgs_.end();
					...
					if (mi->second->hash.dat[1] == MSGTYPE_BLK) { //FIXME, consider checking mi->first
						...
						if (now > mi->second->got + MAX_MSGWAIT
								&& mi->second->msid <= last_srvs_.now) {
							tmp_msgs_.push_back(mi->second);
						}
					} else {
						if (mi->second->msid
								< srvs_.nodes[mi->second->svid].msid) {
							missing_msgs_.erase(mi);
							continue;
						}
						if (now > mi->second->got + MAX_MSGWAIT
				...
				for (auto re = tmp_msgs_.begin(); re != tmp_msgs_.end(); re++) {
					...
					if (svid) {
						fprintf(stderr,
								"HASH request:%016lX [%016lX] (%04X) %d:%d\n",
								(*re)->hash.num, *((uint64_t*) (*re)->data),
								svid, (*re)->svid, (*re)->msid); // could be bad allignment
						//std::cerr << "REQUESTING MESSAGE from "<<svid<<" ("<<(*re)->svid<<":"<<(*re)->msid<<")\n";
						fprintf(stderr,
								"REQUESTING MESSAGE %04X:%08X from %04X\n",
				...
				for (auto wa = wait_msgs_.begin(); wa != wait_msgs_.end();) {
					if ((*wa)->now < srvs_.now + BLOCKSEC
							&& srvs_.nodes[(*wa)->svid].msid
									== (*wa)->msid - 1) {
				...
				wait_.unlock();
				check_.lock();
				check_msgs_.insert(check_msgs_.end(), tmp_msgs_.begin(),
						tmp_msgs_.end());
				check_.unlock();
				...
				if (srvs_.nodes[msg->svid].msid >= msg->msid) {
					...
					fprintf(stderr,
							"WARNING ignoring validation of old message %04X:%08X (<=%08X)\n",
				...
				//std::cerr <<"WARNING postponing validation of future message "<<msg->svid<<":"<<msg->msid<<"!="<<srvs_.nodes[msg->svid].msid<<"+1\n";
					fprintf(stderr,
							"WARNING postponing validation of future message %04X:%08X (!=%08X+1)\n",
							msg->svid, msg->msid, srvs_.nodes[msg->svid].msid);
					wait_.lock();
	...
	bool remove_message(message_ptr msg) // log removing of message
		...
		while (p < (uint8_t*) msg->data + msg->len) {
			...
			if ((*p == TXSTYPE_PUT || *p == TXSTYPE_GET)
					&& utxs.bbank == opts_.svid) {
	...
	bool undo_message(message_ptr msg) {
		...
		if (users) {
			if (srvs_.nodes[msg->svid].users != users) {
				fprintf(stderr,
						"WARNING undoing user additions (users back to:%08X)\n",
						users);
			}
		...
		for (auto it = undo.begin(); it != undo.end(); it++) {
			user_t& u = it->second;
			fprintf(stderr,
					"UNDO:%04X:%08X m:%08X t:%08X s:%04X b:%04X u:%08X l:%08X r:%08X v:%016lX\n",
					msg->svid, it->first, u.msid, u.time, u.stat, u.node,
					u.user, u.lpath, u.rpath, u.weight);
	...
	bool process_message(message_ptr msg) {
		...
		while (p < (char*) msg->data + msg->len) {
			...
			//std::cerr<<"ERROR: time in the future block\n";
				fprintf(stderr,
						"ERROR: time in the future block time:%08X block:%08X limit %08X\n",
			...
			if (*p == TXSTYPE_USR) { // check lock first
				...
				if (luser < users) { //1. check if overwriting was legal
					...
					if (usera->weight - TIME_FEE(lpath, usera->lpath) >= 0) {
						...
						fprintf(stderr,
								"WARNING, overwriting account %04X:%08X [weight:%016lX fee:%08X]\n",
								utxs.bbank, luser, usera->weight,
								TIME_FEE(lpath, usera->lpath));
				...
				srvs_.init_user(*usera, msg->svid, luser,
						(utxs.abank == utxs.bbank ? MIN_MASS : 0),
						(uint8_t*) npkey, utxs.ttime);
			...
			//  return(false);}
			if (utxs.wrong_sig((uint8_t*) p, (uint8_t*) usera->hash,
					(uint8_t*) usera->pkey)) {
			...
			if (usera->time + 2 * LOCK_TIME < lpath && usera->user
					&& usera->node
					&& (usera->user != utxs.auser || usera->node != utxs.abank)) { //check account lock
				if (*p != TXSTYPE_PUT || utxs.abank != utxs.bbank
						|| utxs.auser != utxs.buser || utxs.tmass != 0) {
					std::cerr
							<< "ERROR: account locked, send 0 to yourself and wait for unlock\n";
					return (false);
				}
			}
			...
			//lnode=usera->lnode;
				...
				if (utxs.auser) {
					...
					fprintf(stderr,
							"ERROR: bad user %04X for this bank changes\n",
							utxs.auser);
					return (false);
				}
			...
			if (deduct + fee + MIN_MASS > usera->weight) {
				...
				fprintf(stderr,
						"ERROR: too low balance txs:%016lX+fee:%016lX+min:%016lX>now:%016lX\n",
		...
		if (set_bky) {
			...
			if (msg->svid == opts_.svid) {
				if (!srvs_.find_key(new_bky, skey)) {
					fprintf(stderr,
							"ERROR, failed to change to new bank key, fatal!\n");
					exit(-1);
				}
			}
		}
	...
	void commit_block(std::set<uint16_t>& update) //assume single thread
		...
		if (!blk_bnk.empty()) {
			...
			uint16_t peer = 0;
			for (auto it = srvs_.nodes.begin() + 1; it != srvs_.nodes.end();
					it++, peer++) { // start with bank=1
				if (it->mtim + BANK_MIN_MTIME < srvs_.now
						&& it->weight < BANK_MIN_WEIGHT) {
...
//FIXME !!!
				...
				update.insert(abank);
				for (auto tx = it->second.begin(); tx != it->second.end();
						tx++) {
					user_t u;
		...
		for (auto it = deposit.begin(); it != deposit.end(); it++) {
			...
			if (svid != lastsvid) {
				...
				if (fd < 0) {
					//std::cerr<<"ERROR, failed to open bank register "<<svid<<", fatal\n";
					fprintf(stderr,
							"ERROR, failed to open bank register %04X, fatal\n",
							svid);
					exit(-1);
				}
				sprintf(filename, "blk/%08X/und/%04X.dat", srvs_.now, svid);
				...
				if (ud < 0) {
					//std::cerr<<"ERROR, failed to open bank undo "<<svid<<", fatal\n";
					fprintf(stderr,
							"ERROR, failed to open bank undo %04X, fatal\n",
							svid);
					exit(-1);
				}
			}
...
//FIXME
	...
	void update_list(std::vector<uint64_t>& txs, std::vector<uint64_t>& dbl,
			uint16_t peer_svid) {
		txs_.lock();
	...
	void finish_block() {
		...
		for (auto it = winner->svid_have.begin(); it != winner->svid_have.end();
				it++) {
...
//FIXME, include dblspend message search
			...
			while (1) {
				if ((mi == txs_msgs_.end())
						|| ((mi->first & 0xFFFFFFFFFFFF0000L) != (h.num))) {
					fprintf(stderr,
							"FATAL, could not find svid_have message :-( %016lX\n",
							h.num);
					exit(-1);
				}
		...
		for (auto it = winner->svid_miss.begin(); it != winner->svid_miss.end();
				it++) {
			uint16_t svid = it->first;
			uint32_t msid = it->second.msid;
			if (!msid) {
...
//FIXME, include dblspend message search
			...
			while (1) {
				if ((mi == txs_msgs_.end())
						|| ((mi->first & 0xFFFFFFFFFFFF0000L) != (h.num))) {
					fprintf(stderr,
							"FATAL, could not find svid_miss message :-( %016lX\n",
							h.num);
					exit(-1);
				}
				if (!memcmp(sigh, mi->second->sigh, sizeof(hash_t))) {
		...
		for (auto it = last_block_svid_msgs.begin();
				it != last_block_svid_msgs.end(); it++) {
		...
		if (ca == candidates_.end()
				|| memcmp(last_block_message.hash, ca->first.hash,
						sizeof(hash_t))) {
		...
		for (auto mj = txs_msgs_.begin(); mj != txs_msgs_.end();) { // adding more messages, TODO this should not be needed ... all messages should have path=srvs_.now
			...
			//std::cerr << "REMOVE message " << nsvid << ":" << mi->second->msid << " later ! (DBL-spend server)\n";
				fprintf(stderr,
						"REMOVE message %04X:%08X later! (DBL-spend server)\n",
			...
			if (mi->second->msid > maxmsid) {
				...
				if (mi->second->len == message::header_length) {
					...
					fprintf(stderr, "IGNOR: %04X:%08X %.16s %016lX (%016lX)\n",
							nsvid, mi->second->msid, hash, mi->second->hash.num,
							mi->first);
//???, what about emsid ???
					continue;
				}
				...
				if (mi->second->path != srvs_.now) {
					ed25519_key2text(hash, mi->second->sigh, sizeof(hash_t));
					fprintf(stderr,
							"PATH?: %04X:%08X %.16s %016lX (%016lX) path=%08X srvs_.now=%08X\n",
							nsvid, mi->second->msid, hash, mi->second->hash.num,
							mi->first, mi->second->path, srvs_.now);
				}
				...
				if (maxmsid != 0xffffffff && mi->second->msid != minmsid) {
					...
					fprintf(stderr,
							"ERROR, failed to read correct transaction order in block %04x:%08X<>%08X (max:%08X)\n",
							nsvid, mi->second->msid, minmsid, maxmsid);
					for (auto mx = txs_msgs_.begin(); mx != txs_msgs_.end();
							mx++) {
				...
				fprintf(stderr, "BLOCK: %d %d %.16s %016lX (%016lX)\n", nsvid,
						mi->second->msid, hash, mi->second->hash.num,
						mi->first);
				fprintf(fp, "%d %d %.*s\n", nsvid, mi->second->msid,
						(int) (2 * sizeof(hash_t)), hash);
			}
...
//TODO
	...
	void update_candidates(message_ptr msg) {
		...
		if (do_block > 0) { // update candidates, check if this message was not missing
			for (auto it = candidates_.begin(); it != candidates_.end(); it++) {
				...
				if (m != it->second->svid_miss.end()
						&& m->second.msid == msg->msid
						&& !memcmp(m->second.sigh, msg->sigh, sizeof(hash_t))) {
	...
	message_ptr write_handshake(uint16_t peer, handshake_t& hs) {
		...
		if (peer) {
			if (!do_sync) {
				...
				memcpy(hs.msha, last_srvs_.nodes[peer].msha,
		...
		hash_t empty;
		message_ptr msg(
				new message(MSGTYPE_INI, (uint8_t*) &hs,
						(int) sizeof(handshake_t), opts_.svid, msid_, skey,
						pkey, empty));
		return (msg);
	}
	...
	uint32_t write_message(std::string line) // assume single threaded
		...
		message_ptr msg(
				new message(MSGTYPE_TXS, (uint8_t*) line.c_str(),
						(int) line.length(), opts_.svid, msid, skey, pkey,
						srvs_.nodes[opts_.svid].msha));
		if (!txs_insert(msg)) {
			std::cerr
					<< "FATAL message insert error for own message, dying !!!\n";
			exit(-1);
		}
		return (msid_);
		//update(msg);
	...
	void write_candidate(const hash_s& last_message) {
		...
		hash_t empty;
		message_ptr msg(
				new message(MSGTYPE_CND, last_message.hash, sizeof(hash_t),
						opts_.svid, srvs_.now, skey, pkey, empty)); //FIXME, consider msid=0 ???
//FIXME, is hash ok ?
		if (!cnd_insert(msg)) {
			std::cerr
					<< "FATAL message insert error for own message, dying !!!\n";
	...
	void write_header() {
		...
		hash_t empty;
		message_ptr msg(
				new message(MSGTYPE_BLK, (uint8_t*) &head, sizeof(header_t),
						opts_.svid, head.now, skey, pkey, empty));
		if (!blk_insert(msg)) {
			std::cerr
					<< "FATAL message insert error for own message, dying !!!\n";
			exit(-1);
...
//FIXME, save only if I am important
	...
	void clock() {
		...
		while (1) {
			...
			if (missing_msgs_.size()) {
				fprintf(stderr,
						"CLOCK: %02lX (check:%d wait:%d peers:%d hash:%8X) (miss:%d:%016lX)\n",
						((long) (srvs_.now + BLOCKSEC) - (long) now),
						(int) check_msgs_.size(), (int) wait_msgs_.size(),
						(int) peers_.size(),
						(uint32_t) *((uint32_t*) srvs_.nowhash),
						(int) missing_msgs_.size(),
						missing_msgs_.begin()->first);
			} else {
				fprintf(stderr,
						"CLOCK: %02lX (check:%d wait:%d peers:%d hash:%8X)\n",
						((long) (srvs_.now + BLOCKSEC) - (long) now),
						(int) check_msgs_.size(), (int) wait_msgs_.size(),
						(int) peers_.size(),
						(uint32_t) *((uint32_t*) srvs_.nowhash));
	...
	bool break_silence(uint32_t now, std::string& message) // will be obsolete if we start tolerating empty blocks
		...
		if (!do_block && do_hallo != srvs_.now
				&& now - srvs_.now
						> (uint32_t) (BLOCKSEC / 4 + opts_.svid * VOTE_DELAY)
				&& svid_msgs_.size() < MIN_MSGNUM) {
...
//};
	...
	void start_accept();
	void handle_accept(peer_ptr new_peer,
